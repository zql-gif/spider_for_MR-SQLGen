...
def zhipu_text_to_sql_agent_DAIL_SQL(client, temperature, model, llm_key, content):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "Let's think step by step.You are an expert in sqls."},
            {"role": "user", "content": formatted_target_prompt}
        ],
    )
    response_content = response.choices[0].message.content.replace("```", "").replace("json", "").replace("\n", " ").replace("sql", "").strip()
    # _, response_json = try_parse_json_object(response_content)
    return {"sql":response_content}


def zhipu_text_to_sql_process(gold_input_json, gold_tables_json, test_cnt, model, exp_id, temperature, llm_key):
    ...
    while finished_cnt < test_cnt and finished_cnt < len(contents):
        ...
        response = zhipu_text_to_sql_agent_DAIL_SQL(chat, temperature, model, llm_key, contents[finished_cnt])
        print(response)
